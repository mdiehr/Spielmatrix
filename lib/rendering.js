...
(function(mod) {
	...
	Spielmatrix.rendering = {
		...
		initializeRenderer : function(sm) {
			...
			// PIXI.BaseTexture.SCALE_MODE.DEFAULT = PIXI.SCALE_MODES.NEAREST;
		...
		initializeTiles : function(sm) {
			...
			sm.state.glyphTexture = new PIXI.Texture(Spielmatrix.assets.petscii.texture, new PIXI.Rectangle(10*8,13*8,8,8));
			...
			for (var y = 0; y < h; ++y) {
				for (var x = 0; x < w; ++x) {
			 		...
			 		var glyphSprite = new PIXI.Sprite(sm.state.glyphTexture);
		   			sm.state.pixiContainer.addChild(squareSprite);
		   			sm.state.pixiContainer.addChild(glyphSprite);
					sm.state.tiles.push(new Spielmatrix.Tile(squareSprite, glyphSprite, x, y, size, color))
				}
			}
		},
