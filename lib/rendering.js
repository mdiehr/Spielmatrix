	...
	Spielmatrix.rendering = {
		...
		calculateViewSize : function(sm) {
			...
			if (sm.options.pixelPerfect) {
				...
				// Fit tiles to render width
				sm.state.tileSize = sm.state.renderWidth / sm.state.width;
			}
		...
		update : function(sm, data) {
			...
			if (sm.options.resizeCanvas && sm.state.renderWidthMax !== sm.state.renderWidthMin) {
				...
				if (windowWidth < sm.state.renderWidth && sm.state.renderWidth > sm.state.renderWidthMin) {
					resized = true;
					sm.state.renderWidth = Math.floor(Math.max(windowWidth, sm.state.renderWidthMin));
				}

				if (windowWidth > sm.state.renderWidth && sm.state.renderWidth < sm.state.renderWidthMax) {
					resized = true;
					sm.state.renderWidth = Math.floor(Math.min(windowWidth, sm.state.renderWidthMax));
				}

				if (resized) {
					var width = Math.floor(sm.state.renderWidth);
					var height = Math.floor(sm.state.height * (width / sm.state.width));
					sm.state.renderHeight = height;
					sm.state.pixiContainer.width = width;
					sm.state.pixiContainer.height = height;
					sm.state.renderer.resize(width, height);
